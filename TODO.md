...
- ssh back into chrome os via `~/.ssh/config`:
  ...
  - ```
    Host jumphost
      ...
      ProxyCommand sh -c 'cd /tmp ; busybox fuser -4 2222/tcp || screen -dmS ssh env -i /usr/local/crosware/software/dropbear/current/sbin/dropbear -m -R -F -E -B -P 2222 -s -a -G chronos-access; busybox nc %h %p'
    ```
