...
- memoization in `cwsourceprofile`
  - `cwsourcerecipes` has to have been run at least once to start memoization
    - check on `cwrecipessourced=1` and don't set our sentinel until it's detected
    - may have to do multiple full source ins...
  - each recipe has a ```cwprof_${rname}```
  - run through FULL source of `${cwtop}/etc/profile`
  - memoize that
  - gen a hash of ALL valid `${cwtop}/etc/profile.d/${rname}.sh` files
    - key 0 if it doesn't exist
    - key ```sha256sum $(cwprof_${rname})``` if it does
  - subsequent runs test only if keys == 0 have showed up
    - if so, source it in, and update its hash
  - for any key != 0, check on-disk sha256sum...
    - compare to that sum stored for key
    - if they don't match, source it in, and update its hash
  - may need another sentinel?
  - ```
      ...
      if [ ${cwprofileinit} -eq 0 ] ; then
        if [ ${cwrecipessourced} -eq 0 ] ; then
          ...
          for r in ${!cwrecipes[@]} ; do
            f=$(cwprof_${r})
            test -e ${f} && cwprofhash[${f}]=$(sha256sum ${f} | awk '{print $1}') || cwprofhash[${f}]=0
          done
