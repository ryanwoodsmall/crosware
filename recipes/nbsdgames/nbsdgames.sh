...
function cwpatch_${rname}() {
  ...
  sed -i \"/^LDFLAGS/s,^.*,LDFLAGS=-L${cwsw}/netbsdcurses/current/lib -lcurses -lterminfo -static -s,g\" src/Makefile
  sed -i \"/^CFLAGS/s|= |= -Os -I${cwsw}/netbsdcurses/current/include -Wl,-L${cwsw}/netbsdcurses/current/lib -Wl,-static -Wl,-s |g\" src/Makefile
  sed -i \"/^LIBS_PKG_CONFIG/s,^.*,LIBS_PKG_CONFIG=-L${cwsw}/netbsdcurses/current/lib -lcurses -lterminfo -static -s,g\" src/Makefile
  grep -rl -- -lncurses \"\$(cwbdir_${rname})/\" | grep -v ORIG\$ | xargs sed -i.NCURSES 's,-lncurses,-lcurses -lterminfo,g'
  popd &>/dev/null
}
"
...
function cwmake_${rname}() {
  ...
  (
    ...
    export LDFLAGS=\"-L${cwsw}/netbsdcurses/current/lib -lcurses -lterminfo -static -s\"
