...
function cwmake_${rname}() {
  ...
  cd cmd
  env PLAN9=\"${rbdir}\" PATH=\"\${PATH}:${rbdir}/bin\" \"${cwsw}/9base/current/bin/mk\" 9p.install 9pserve.install CC9=\"\${CC}\"
  cd 9pfuse
...
function cwmakeinstall_${rname}() {
  ...
  install -m 0755 bin/9pserve \"${ridir}/bin/\"
  install -m 0755 bin/9pfuse \"${ridir}/bin/\"
  install -m 0644 man/man1/9p.1 \"${ridir}/man/man1/\"
  install -m 0644 man/man4/9pserve.4 \"${ridir}/man/man4/\"
